...
class VLSTuRF(BaseEstimator, TransformerMixin):
    ...
    def __init__(self, core_algorithm, n_features_to_select=10, n_neighbors=100, pct=0.5, n_way_interaction=2, size_feature_subset=5, discrete_threshold=10, max_iter = 1, verbose=False, n_jobs=1):
        ...
        n_way_interaction: int (default: 2)
        	number of predictive variables. Used to calculate the number of subsets for VLS run in each TuRF iteration
        ...
        self.pct = pct
        self.n_way_interaction = n_way_interaction
    ...
    def fit(self, X, y, headers):
        ...
        while(iter_count < iterMax):
            ...
            vlsNumFeature = 4 * (int(num_features / self.size_feature_subset) ** self.n_way_interaction) # number specified in the VLS paper
            fs = VLSRelief(core_algorithm=self.core_algorithm, n_features_to_select= self.n_features_to_select, n_neighbors = self.n_neighbors,
                num_feature_subset = vlsNumFeature , size_feature_subset = self.size_feature_subset, 
                verbose=self.verbose, n_jobs = self.n_jobs)
            fs.fit(self.X_mat, self._y, self.headers)
        
            feature_column, feature_weights = zip(*fs.feat_score)
            feature_column, feature_weights = np.asarray(feature_column), np.asarray(feature_weights)

            features_iter.append(feature_weights)  # HISTORY
            ...
            #dummy check so feature_retain >= 1

            #scale size_feature_subset appropriately
            self.size_feature_subset = int(np.round(self.size_feature_subset*perc_retain))
